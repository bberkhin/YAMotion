...
CCoordMotion::CCoordMotion(ILogger *plog, IOutput *_hrout) : logger(plog), hrout(_hrout)
{
					   ...
					   char XSTR, char YSTR, char ZSTR, std::string &errmsg)
{
	...
	do
	{
		...
		if (x_axis>=0)
		{
			if (x > SoftLimitPosX+SIGMA) {errmsg=std::string(1,XSTR)+ std::string("+"); return 1;}
			if (x < SoftLimitNegX-SIGMA) {errmsg= std::string(1,XSTR)+ std::string("-"); return 1;}
		}
		if (y_axis>=0)
		{
			if (y > SoftLimitPosY+SIGMA) {errmsg= std::string(1,YSTR)+ std::string("+"); return 1;}
			if (y < SoftLimitNegY-SIGMA) {errmsg= std::string(1,YSTR)+ std::string("-"); return 1;}
		}
	...
	if (z_axis>=0)
	{
		if (z > SoftLimitPosZ) {errmsg= std::string(1,ZSTR)+std::string("+"); return 1;}
		if (z < SoftLimitNegZ) {errmsg= std::string(1, ZSTR)+ std::string("-"); return 1;}
	}
	if (a_axis>=0)
	{
...
int CCoordMotion::StraightTraverse(double x, double y, double z, double a, double b, double c, double u, double v, bool NoCallback, int sequence_number, int ID)
{
	...
	if (CheckSoftLimits(x,y,z,a,b,c,u,v,errmsg)) 
	{
		if (m_Simulate)
		{
			logger->log(LOG_ERROR, "Soft Limit %s Rapid Traverse\r\r Soft Limits disabled for remainder of Simulation", errmsg.c_str());
		...
		else
		{
			...
			SetAbort();
			logger->log(LOG_ERROR, "Soft Limit %s Rapid Traverse Job Halted", errmsg.c_str());
			return 1;
		}
	}
...
int CCoordMotion::ArcFeed(double DesiredFeedRate_in_per_sec, Plane plane,
...
int CCoordMotion::ArcFeedAccel(double DesiredFeedRate_in_per_sec, double DesiredAccel, Plane plane,
	double first_end, double second_end,
	double first_axis, double second_axis, int rotation,
	double axis_end_point, double a, double b, double c, int sequence_number, int ID)
{
				...
				double axis_end_point, double a, double b, double c, double u, double v, int sequence_number, int ID)
{
	...
	if (cord < MP->CollinearTol)
	{
		...
		else if (plane == Plane_XZ) // actually ZX
	...
	if (m_ArcFeedCallback) m_ArcFeedCallback(true, 
				...
				first_axis, second_axis, rotation,
				axis_end_point,
				cur_first,cur_second,cur_third, _setupsequence_number, ID);
	...
	if (SoftLimitResult) 
	{
		if (m_Simulate)
		{
			logger->log( LOG_ERROR,"Soft Limit %s Arc\r\r Soft Limits disabled for remainder of Simulation", errmsg.c_str() );
			m_DisableSoftLimits=true;
		}
		else
		{
			...
			CheckMotionHalt(true);
			SetAbort();
			logger->log(LOG_ERROR, "Soft Limit %s Arc Job Halted", errmsg.c_str());
			return 1;
		}
	}
							   ...
							   double x, double y, double z, double a, double b, double c, double u, double v, int sequence_number, int ID)
{
	...
	{
		SetAbort();
		logger->log(LOG_ERROR, "Straight Feed with Feed Rate Zero or Negative");		
		return 1;
	}
	...
	if (CheckSoftLimits(x,y,z,a,b,c,u,v,errmsg)) 
	{
		...
		if (m_Simulate)
		{
			logger->log(LOG_ERROR, "Soft Limit %s %s \r\rSoft Limits disabled for remainder of Simulation", errmsg.c_str(), FunctionType.c_str());
			m_DisableSoftLimits=true;
		}
		else
		{
			...
			CheckMotionHalt(true);
			SetAbort();
			logger->log(LOG_ERROR, "Soft Limit %s %s Job Halted", errmsg.c_str(), FunctionType.c_str());
			return 1;
		}
	}
			...
			if (i>=0)
			{
				...
				if (p->type == SEG_LINEAR)
				{
					if (m_StraightTraverseCallback)
						if (m_StraightTraverseCallback) m_StraightTraverseCallback(p->x1,p->y1,p->z1,_setupsequence_number);
...
int CCoordMotion::Dwell(double seconds, int sequence_number)
{
	...
	if (m_StraightTraverseCallback) m_StraightTraverseCallback(current_x, current_y, current_z,_setupsequence_number);
	if (m_StraightTraverseSixAxisCallback) m_StraightTraverseSixAxisCallback(current_x, current_y, current_z, current_a, current_b, current_c,_setupsequence_number);
...
int CCoordMotion::DoKMotionCmd(const char *s, bool FlushBeforeUnbufferedOperation)
{
	...
	if (hrout->WriteLine(s)) {SetAbort(); return 1;}
...
{
	strncpy(special_cmds[nspecial_cmds % MAX_SPECIAL_CMDS].cmd,s,MAX_GCODE_LINELEN);
...
int CCoordMotion::WaitForSegmentsFinished(bool NoErrorOnDisable)
{
	...
	do
	{
		if (count++)
		{
			if (hrout->WriteLineReadLine("ExecTime",response)) { SetAbort(); return 1; }
			if (sscanf(response.c_str(), "%lf",&m_TimeAlreadyExecuted)!= 1){SetAbort(); return 1;}
			UpdateRealTimeState(m_TimeAlreadyExecuted);
			sleep_for(10ms);
		}
		
		if (hrout->WriteLineReadLine("CheckDoneBuf",response)) {SetAbort(); return 1;}

		if (response == "-1")
		{
			if (hrout->WriteLineReadLine("ExecTime",response2)){SetAbort(); return 1;}
			if (sscanf(response2.c_str(), "%lf",&m_TimeAlreadyExecuted)!= 1){SetAbort(); return 1;}
			UpdateRealTimeState(m_TimeAlreadyExecuted);
...
int CCoordMotion::WaitForMoveXYZABCFinished()
{
	...
	do
	{
		if (count++) sleep_for(10ms);
		if (hrout->WriteLineReadLine("CheckDoneXYZABC",response)) {SetAbort(); return 1;}
...
{
	std::string response,responsebuf;
	double BufTime=0;
	...
	{
		// stop immediately
		if (hrout->WriteLine("StopImmediate0"))  {SetAbort(); return 1;}

		// wait until stopped
		do
		{
			if (hrout->WriteLineReadLine("GetStopState",response))  {SetAbort(); return 1;}
			if (m_Abort) return 1;
			...
			if (Coord && response == "1")
			{
				if (hrout->WriteLineReadLine("CheckDoneBuf",responsebuf)) {SetAbort(); return 1;}
		...
		//bb 
		int _setupsequence_number = 1;
		m_PreviouslyStoppedSeqNo = _setupsequence_number;
		...
		if (Coord)
		{
			...
			else
			{
				...
				if (hrout->WriteLineReadLine("ExecTime",response)) return 1;
				int result=sscanf(response.c_str(), "%lf",&m_TimeAlreadyExecuted);
		...
		if (hrout->WriteLine("StopImmediate2"))  {SetAbort(); return 1;}
...
int CCoordMotion::FlushSegments()
{
	...
	if (nsegs>0 || nspecial_cmds>0)
	{
		if (m_nsegs_downloaded==0)
		{
			...
			if(hrout->WriteLine("OpenBuf")) {SetAbort(); return 1;}
			m_TimeAlreadyExecuted=0;
		...
		if(hrout->WriteLine("FlushBuf")) {SetAbort(); return 1;}
...
int CCoordMotion::LaunchCoordMotion()
{
	...
	//if (KMotionDLL->WriteLineReadLine("CheckDoneBuf",s.GetBufferSetLength(MAX_GCODE_LINELEN))) {SetAbort(); return 1;}
	//s.ReleaseBuffer();
...
int CCoordMotion::OutputSegment(int iseg)
{
	...
	if (m_SegmentsStartedExecuting)
	{
		...
		while (wait && m_SegmentsStartedExecuting)
		{
			...
			if (hrout->WriteLineReadLine("ExecTime",response)) return 1;
			result=sscanf(response.c_str(), "%lf",&m_TimeAlreadyExecuted);
			if (result != 1)  return 1;
...
int CCoordMotion::GetAxisDefinitions(int *x, int *y, int *z, int *a, int *b, int *c)
{
	...
	if (!m_DefineCS_valid)
	{
		if (hrout->WriteLineReadLine("DefineCS",response)) return 1;
		result=sscanf(response.c_str(), "%d%d%d%d%d%d",&x_axis,&y_axis,&z_axis,&a_axis,&b_axis,&c_axis);
...
int CCoordMotion::GetAxisDefinitions(int *x, int *y, int *z, int *a, int *b, int *c, int *u, int *v)
{
	...
	if (!m_DefineCS_valid)
	{
		if (hrout->WriteLineReadLine("DefineCSEx",response)) return 1;
		result = sscanf(response.c_str(), "%d%d%d%d%d%d%d%d", &x_axis, &y_axis, &z_axis, &a_axis, &b_axis, &c_axis, &u_axis, &v_axis);
...
int CCoordMotion::GetRapidSettings()
{
	...
	if (m_Simulate)
	{
		int result= hrout->WaitToken(false,100,"GetRapidSettings");
	...
	hrout->WaitToken("GetRapidSettings2");  // lock the Token while we get all the parameters
...
int CCoordMotion::GetRapidSettingsAxis(int axis,double *Vel,double *Accel,double *Jerk, double *SoftLimitPos, double *SoftLimitNeg, double CountsPerInch)
{
	...
	if (hrout->ReadLineTimeOut(response)) return 1;
	result=sscanf(response.c_str(), "%lf",&temp);
	if (result != 1) return 1;
...
int CCoordMotion::GetDestination(int axis, double *d)
{
	...
	*d=0.0;
	...
	if (hrout->WriteLineReadLine(cmd,response)) {SetAbort(); return 1;}
	result=sscanf(response.c_str(), "%lf",d);
	if (result != 1) {SetAbort(); return 1;}
...
int CCoordMotion::GetAxisDone(int axis, int *r)
{
	...
	*r=-1;  // assume disabled
	...
	if (hrout->WriteLineReadLine(cmd,response)) {SetAbort(); return 1;}
	result=sscanf(response.c_str(), "%d",r);
	if (result != 1) {SetAbort(); return 1;}
...
int CCoordMotion::MeasurePointAppendToFile(const char *name)
{
	...
	if (!f)
	{
		logger->log(LOG_ERROR,"Unable to open Geometric Correction File : %s", name);
		return 1;
	}
	...
	{
		fclose(f);
		logger->log(LOG_ERROR, "Invalid Geometric Correction File (NRows and NCols) : %s", name);
		return 1;
	}
	...
	{
		fclose(f);
		logger->log(LOG_ERROR, "Invalid Geometric Correction File (GeoSpacingX and GeoSpacingY) : %s",name);
		return 1;
	}
	...
	{
		fclose(f);
		logger->log(LOG_ERROR, "Invalid Geometric Correction File (GeoOffsetX and GeoOffsetY) : %s", name);
		return 1;
	}
	...
	if (!f)
	{
		logger->log(LOG_ERROR, "Unable to open Measurement Point File : %s", name);
		return 1;
	}
...
int CCoordMotion::PutWriteLineBuffer(std::string s, double Time)
{
	...
	if (WriteLineBuffer.size() + s.size() > MAX_GCODE_LINELEN-10)
	{
...
{
	if (m_Abort) return 1;
	int result = hrout->WriteLine(WriteLineBuffer.c_str());
	ClearWriteLineBuffer();
	return result;
}
...
int CCoordMotion::GetSpindleRPS(float &speed)
{
	...
	if (hrout->WriteLineReadLine("GetSpindleRPS",response)) return 1;
	
	// check state
	if (sscanf(response.c_str(),"%f",&speed)!=1) return 1;
