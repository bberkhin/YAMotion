...
void RoundCorner(int is)
{
	...
	double B = HALF_PI - Theta2/2.0;

	double Q = (PI - Theta)/2.0;

	double D = HALF_PI - B + Q;
...
int tp_insert_arc_seg(Plane plane,
...
double Pos(TP_COEFF *p)
{                  
	return ((p->a * p->t + p->b)*p->t + p->c)*p->t + p->d;
}
...
double Vel(TP_COEFF *p)
{
	return (3*p->a * p->t + 2*p->b)*p->t + p->c;
}
